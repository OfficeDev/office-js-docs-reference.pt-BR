...
|12002|||Uma destas opções:<br> - Não existe uma página na URL transmitida para `displayDialogAsync`.<br> - A página transmitida para `displayDialogAsync` foi carregada, mas a caixa de diálogo foi direcionada para uma página que ela não consegue localizar nem carregar ou foi direcionada para uma URL com sintaxe inválida. Lançada dentro do diálogo e trigges um `DialogEventReceived` evento na página host.|
|12003|||A caixa de diálogo foi direcionada para uma URL com o protocolo HTTP. HTTPS é necessário. Lançada dentro do diálogo e trigges um `DialogEventReceived` evento na página host.|
|12004|||O domínio da URL é passado para `displayDialogAsync` não é confiável. O domínio deve ser do mesmo domínio que a página de host (incluindo o número da porta e protocolo). Lançada por chamada de `displayDialogAsync`.|
|12005|||A URL passado para `displayDialogAsync` usa o protocolo HTTP. HTTPS é necessário. Lançada por chamada de `displayDialogAsync`. (Em algumas versões do Office, a mensagem de erro retornada com 12005 é o mesmo retornada por uma para 12004.)|
|12006|||A caixa de diálogo foi fechada, geralmente pelo usuário ter escolhido o botão **X**. Lançada dentro do diálogo e trigges um `DialogEventReceived` evento na página host.|
|12007|||Uma caixa de diálogo já está aberta dessa janela host. Uma janela do host, como um painel de tarefas, só pode ter uma caixa de diálogo Abrir ao mesmo tempo. Lançada por chamada de `displayDialogAsync`.|
|13000 - 13010|||Consulte [causas e tratamento de erros de getAccessTokenAsync](https://docs.microsoft.com/office/dev/add-ins/develop/troubleshoot-sso-in-office-add-ins#causes-and-handling-of-errors-from-getaccesstokenasync).|
...
|Table|Texto|Não é possível criar a associação.|
