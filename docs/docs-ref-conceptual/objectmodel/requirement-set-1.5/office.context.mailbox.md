...
> [!NOTE]
> Clientes do Outlook conectados a instalações de local do Exchange 2016 ou posteriores com uma URL personalizada do REST configurado retornará um valor inválido para `restUrl`.
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook_1_5/office.mailboxenums.restversion)|Um valor que indica a versão da API REST do Outlook usada para recuperar a ID do item.|
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook_1_5/office.mailboxenums.restversion)|Um valor que indica a versão da API REST do Outlook com a qual a ID convertida será usada.|
...
|Nome| Tipo| Descrição|
|---|---|---|
|`input`| [LocalClientTime](/javascript/api/outlook_1_5/office.LocalClientTime)|O valor de hora local a converter.|
...
|Nome| Tipo| Descrição|
|---|---|---|
|`itemId`| Cadeia de caracteres|O identificador dos Serviços Web do Exchange (EWS) para uma mensagem existente.|
...
> [!NOTE]
> É recomendável que os suplementos usam as APIs REST em vez de serviços Web do Exchange sempre que possível. 
...
> - Quando o suplemento for carregado em uma caixa de correio do Gmail
> 
> Nesses casos, suplementos devem [usar APIs REST](https://docs.microsoft.com/outlook/add-ins/use-rest-api) para acessar a caixa de correio do usuário.

O método `makeEwsRequestAsync` envia uma solicitação do EWS em nome do suplemento ao Exchange. Para obter uma lista das operações EWS suportadas, consulte [chame serviços web a partir de um suplemento do Outlook](https://docs.microsoft.com/outlook/add-ins/web-services#ews-operations-that-add-ins-support) .
...
> [!NOTE]
> O administrador do servidor deve ser definida `OAuthAuthentication` como true no diretório do EWS de servidor de acesso de cliente para habilitar o `makeEwsRequestAsync` método para tornar o EWS solicita.
