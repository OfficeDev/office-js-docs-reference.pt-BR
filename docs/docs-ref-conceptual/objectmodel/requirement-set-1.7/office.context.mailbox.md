...
Atualmente, os tipos de evento aceitos são `Office.EventType.ItemChanged` e `Office.EventType.OfficeThemeChanged`.
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook_1_7/office.mailboxenums.restversion)|Um valor que indica a versão da API REST do Outlook usada para recuperar a ID do item.|
...
|Nome| Tipo| Descrição|
|---|---|---|
|`input`| [LocalClientTime](/javascript/api/outlook_1_7/office.LocalClientTime)|O valor de hora local a converter.|
...
| `parameters.attachments.isInline` | Boolean | Usado somente se `type` estiver definido como `file`. Se for `true`, indicará que o anexo será mostrado embutido no corpo da mensagem e não deverá ser exibido na lista de anexos. |
| `parameters.attachments.itemId` | Cadeia de caracteres | Usado somente se `type` estiver definido como `item`. A id de item do EWS do email existente que deseja anexar para a nova mensagem. Isso é uma cadeia de até 100 caracteres. |
